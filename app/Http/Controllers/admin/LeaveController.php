...
class LeaveController extends Controller
{
        ...
        if ($request->ajax()) {
            ...
            return Datatables::of($data)
                    ...
                    ->addColumn('action', function($data){
                           ...
                           $btn = '<a href="' . route('admin.leave.edit_leave', ['id' => $data->id]) . '" class="mx-1"><i class="fadeIn animated bx bx-edit-alt"></i></a>';
                             $btn .= '<a href="javascript:void(0)" data-value = "' . route('admin.leave.delete', ['id' => $data->id]) . '" class="mx-1 delete_package"><i class="fadeIn animated bx bx-trash-alt"></i></a>'; 
                              $btn .= '<a class="mx-1 ajaxviewmodel" href="' . route('admin.leave.leave_status_change', ['id' => $data->id]) . '"><i class="fadeIn animated bx bx-reset"></i></a>';   
                            return $btn;
                    })
    ...
    {
        if ($request->ajax()) {
            $data = leave::with(['user','admin'])->orderby('id','desc')->select('*');
            return Datatables::of($data)
                    ->addIndexColumn()
                     ->addColumn('user', function ($data) {
                            return $data->user->first_name;
    ...
    public function add(){
        ...
        return view('admin.leave.add',['user'=>$user]);
    }
    ...
    public function leave_edit($id)
    {
            ...
            $user= User::where('parent_id',$user_id)->get();
                ...
                return view('admin.leave.edit', ["user"=>$user, "leave"=>$leave]);
    }
    ...
    public function leave_delete($id){
     ...
     public function leave_status_change(Request $request, $id) {
        ...
        return view('admin.leave.changestatus', ['data' => $data]);
    }
